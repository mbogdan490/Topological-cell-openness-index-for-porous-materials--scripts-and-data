...
``Betti_curves/{Sample}_betti.csv``, with per-level 3D tau:
...
Permeability
------------
Uses **permeability** (mD) from ``permeability.csv`` (columns ``sample``,
``permeability``, ``reported_porosity``). The plot x-axis is
log10(permeability) with a log scale spanning the sample range (~9–386 mD).
Subplot annotations show Pearson statistics vs log10(permeability); the
correlations text file reports both linear permeability and log10(permeability)
associations.

Writes ``fig6_permeability_vs_tau.png`` and
``fig6_permeability_vs_tau_correlations.txt``.
"""
...
# Script lives at official_repository/codes/regenerating_plots_from_datasets/fig_6/,
...
def correlation_lines(
    ...
    lines = [
        ...
        "Permeability in mD (Neumann Table 1 / permeability.csv)",
...
def main() -> None:
    """Build the permeability-vs-tau figure from the official dataset CSVs."""
    global _BETTI_DIR, _METRICS_CSV, _PERMEABILITY_CSV, _PLOT_PATH, _CORRELATIONS_TXT

    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--betti-dir", type=Path, default=_BETTI_DIR)
    parser.add_argument("--metrics-csv", type=Path, default=_METRICS_CSV)
    parser.add_argument("--permeability-csv", type=Path, default=_PERMEABILITY_CSV)
    parser.add_argument("--output", type=Path, default=_PLOT_PATH)
    parser.add_argument("--correlations", type=Path, default=_CORRELATIONS_TXT)
    args = parser.parse_args()

    _BETTI_DIR = args.betti_dir
    _METRICS_CSV = args.metrics_csv
    _PERMEABILITY_CSV = args.permeability_csv
    _PLOT_PATH = args.output
    _CORRELATIONS_TXT = args.correlations
